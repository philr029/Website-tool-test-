...
function renderFlow(flow: FlowResult, index: number): string {
  ...
  const siteLabel = flow.siteId ? `<span class="site-tag">${escapeHtml(flow.siteId)}</span>` : '';
  return `
...
/** Group flows by siteId. Flows without siteId use the key "__ungrouped__". */
function groupFlowsBySite(flows: FlowResult[]): Map<string, FlowResult[]> {
  const groups = new Map<string, FlowResult[]>();
  for (const flow of flows) {
    const key = flow.siteId ?? '__ungrouped__';
...
function renderFlowsSection(flows: FlowResult[]): string {
  const groups = groupFlowsBySite(flows);
  const isMultiSite = groups.size > 1 || !groups.has('__ungrouped__');

  if (!isMultiSite) {
    // Single site or no siteId — render flat list as before
    return flows.map((f, i) => renderFlow(f, i)).join('\n');
  }

  // Multi-site: render each group under a site heading
  let offset = 0;
  const sections: string[] = [];
  for (const [siteKey, siteFlows] of groups) {
    const heading = siteKey === '__ungrouped__' ? 'Other' : siteKey;
    const cards = siteFlows.map((f, i) => renderFlow(f, offset + i)).join('\n');
    offset += siteFlows.length;
    sections.push(`
  <div class="site-section">
    <h3 class="site-heading">${escapeHtml(heading)}</h3>
    ${cards}
  </div>`);
  }
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .site-tag { display: inline-block; background: #581c8722; color: #d8b4fe; border: 1px solid #581c87; border-radius: 6px; padding: 0.1rem 0.55rem; font-size: 0.72rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }

    /* Site sections (multi-site runs) */
    .site-section { margin-bottom: 1.5rem; }
    .site-heading { color: #cbd5e1; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.1em; font-weight: 700; padding: 0.4rem 0; border-bottom: 1px solid #334155; margin-bottom: 0.75rem; }
...
<body>
  ...
  <div class="flows">
    <h2>Test Results</h2>
    ${renderFlowsSection(suite.flows)}
  </div>
